...
const SendOfferToPartnerModal = React.forwardRef<
...
>(({ jobId, onSuccess, fullJobResource }, ref) => {
  ...
  const modalSteps = useMemo(
    () => [{ title: "Xác nhận thông tin" }, { title: "Điều khoản công việc" }],
    []
  );
  ...
  useEffect(() => {
    if (confirmInfo) {
      const isClient = fullJobResource?.createdByUserId === userInfo?.userId;
      const userProjectBid = fullJobResource?.userProjectBids?.find(
        (bid) => bid.userId === confirmInfo.userId
      );
      ...
      if (
        ...
        const isValidStartDate = latestDeal.startDate &&
          latestDeal.startDate !== "0000-00-00" &&
          dayjs(latestDeal.startDate).isValid();

        const isValidEndDate = latestDeal.endDate &&
        ...
        values = {
          ...
          startDate: latestDeal.startDate && dayjs(latestDeal.startDate).isValid()
            ? dayjs(latestDeal.startDate)
            : null,
          endDate: latestDeal.endDate && dayjs(latestDeal.endDate).isValid()
        ...
        const isValidStartDate = confirmInfo.startDate &&
          confirmInfo.startDate !== '' &&
          confirmInfo.startDate !== "0000-00-00" &&
          dayjs(confirmInfo.startDate).isValid();

        const isValidEndDate = confirmInfo.endDate &&
          confirmInfo.endDate !== '' &&
          confirmInfo.endDate !== "0000-00-00" &&
          dayjs(confirmInfo.endDate).isValid();

        values = {
          jobName: confirmInfo.projectName,
          partnerName: confirmInfo.partnerName,
          startDate: isValidStartDate ? dayjs(confirmInfo.startDate) : null,
          endDate: isValidEndDate ? dayjs(confirmInfo.endDate) : null,
          jobDescription: confirmInfo.description || "",
          negotiatePrice: confirmInfo.negotiatePrice || fullJobResource?.priceMax,
          numberAccept: confirmInfo.numberAccept || fullJobResource?.numberAccept || 1,
          jobSuggestion: confirmInfo.jobSuggestion || "",
        };
      }
  ...
  async function mapFormValuesToApi(values: any) {
    return {
      user_id: confirmInfo?.userId || 0,
      start_date: values.startDate && dayjs(values.startDate).isValid()
        ? dayjs(values.startDate).format("YYYY-MM-DD")
        : null,
      end_date: values.endDate && dayjs(values.endDate).isValid()
        ? dayjs(values.endDate).format("YYYY-MM-DD")
        : null,
  ...
  const { mutateAsync: sendOffer } = useSendOffer(toNumber(jobId), '');
  ...
  }, [form, close, agreeToTerms, confirmInfo, jobId, onSuccess, sendOffer]);

  const { mutateAsync: confirmOfferMutation } = useConfirmPartner(jobId!, '');
  const handleConfirmOffer = useCallback(async () => {
    ...
    try {
      ...
      if (latestDeal) {
        ...
        dataToSubmit = await mapFormValuesToApi(confirmInfo);
      }
  ...
  // ============ RENDER =============
  const getAlertMessage = (confirmInfo: any, latestDeal: any, userProjectBid: any) => {
    if (
      latestDeal &&
      latestDeal.type === Constants.JOB.OFFER.TYPE.CLIENT &&
      latestDeal.status === Constants.JOB.OFFER.STATUS.NOT_RESPONSE
    ...
    if (
      ...
      (
        ...
        latestDeal.type === Constants.JOB.OFFER.TYPE.PARTNER &&
        latestDeal.status === Constants.JOB.OFFER.STATUS.NOT_RESPONSE &&
        latestDeal.dealStatus === Constants.JOB.OFFER.DEAL_STATUS.NOT_ACCEPTED
      )
    ) {
      ...
      <>
        ...
        <div ref={formScrollRef} style={formContainerStyle}>
          ...
          >
            ...
            <Row gutter={16}>
              <Col span={24}>
                {fullJobResource?.createdByUserId === userInfo?.userId && (
                  <ProjectBidTable
                    ...
                    jobQuery={
                      {
                        data: {
                          ...fullJobResource,
                          userProjectBids: fullJobResource?.userProjectBids?.filter(
                            (bid) => bid.userId === confirmInfo?.userId
                          ),
                        },
  ...
  const renderStep2Content = () => (
    <div>
      <Row gutter={16} style={{ marginBottom: 24 }}>
        <Col xs={24}>
          {pdfUrl && (
            <PDFViewer fileUrl={pdfUrl} onReachEnd={() => setCanAgree(true)} />
          )}
        </Col>
      </Row>
  ...
  const getButtonText = () => {
    // If enabledResend is true (radio "edit" is selected), always show "Gửi đề xuất"
    if (enabledResend) return "Gửi đề xuất";
    ...
    if (
      ...
      (!partnerBid?.userProjectDeals || partnerBid.userProjectDeals.length === 0)
    ) {
      return "Gửi đề xuất";
    }
    ...
    if (currentStep === 0) {
      return (
        <Row justify="center" style={{ marginTop: 24 }}>
          <Button onClick={close} size="large" style={{ marginRight: 12 }}>
            Hủy
          </Button>
          {withThemeRevert(
            <Button
...
SendOfferToPartnerModal.displayName = "SendOfferToPartnerModal";
export default SendOfferToPartnerModal;
