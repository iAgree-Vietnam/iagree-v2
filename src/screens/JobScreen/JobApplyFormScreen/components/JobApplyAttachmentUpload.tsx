...
import {
  ...
  getFileIcon,
  getFileMainColor,
} from "@/src/screens/JobScreen/JobApplyFormScreen/utils/JobApplyFormUtils";
...
interface JobApplyAttachmentUploadProps {
  value?: ViewableFile[];
  onChange?: (fileList: ViewableFile[]) => void;
}

const JobApplyAttachmentUpload: React.FC<JobApplyAttachmentUploadProps> = ({
  value = [],
  onChange,
}) => {
  ...
  const handleBeforeUpload = (file: File) => {
    ...
    if (!isValidExtension) {
      message.error(`Định dạng file "${file.name}" không được hỗ trợ.`);
  ...
  const handleFileChange = ({
    ...
    const processedFileList = filteredFileList.map((file) => {
    ...
    <>
      {fileList.length === 0 ? (
        ...
        >
          ...
          >
            (*) Hỗ trợ .pdf, .jpeg, .png, .jpg, .gif, .svg, .doc, .docx. Số lượng tối đa 5 tệp. Dung lượng tệp tối đa 5MB.
          </Text>
        </div>
      ) : (
        <>
...
export default JobApplyAttachmentUpload;
