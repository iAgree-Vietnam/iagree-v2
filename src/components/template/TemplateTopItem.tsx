...
type TemplateTopItemProps = {
    data: TemplateResource;
    onPreview: (templateResource: TemplateResource) => void;
    onReactionSuccess?: () => void;
};

function TemplateTopItem(props: TemplateTopItemProps) {
    const { isDesktop } = useBreakpoint();
    const [templateResource, setTemplateResource] = useState(props.data);

    const reactionMutation = useTemplateReaction({
        onSuccess: () =>
            setTemplateResource({
                ...templateResource,
                isLiked: !templateResource.isLiked,
            }),
    });
...
export default TemplateTopItem;
