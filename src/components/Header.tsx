...
import { compact, size, toString, uniq } from "lodash";
import { ReferralCode } from "./RefferalCode";
import useNotify from "../hooks/useNotify";
import { updateNumberChat } from "../store/slices/notify";
...
const Header = () => {
  ...
  const { headerMenu } = useHeaderMenu({
    isActuallyLoggedIn: isActuallyLoggedIn || false,
    fullProfileResource,
  });
  ...
  const storeNotify = useAppSelector((store) => store.notify);

  const dispatch = useDispatch();
  const chatChannelName = `send.notification.message.${fullProfileResource?.userId}`;
  useNotify(
    fullProfileResource?.userId || 0,
    (n) => {
      const isMessage = StringUtils.isJsonString(n?.description);

      if (isMessage) {
        dispatch(updateNumberChat(storeNotify.numberChat));
      }
    },
    chatChannelName
  );
  const logoutMutation = useLogout({
    onSuccess: () => {
      ...
      if (router.pathname === "/") {
        message.success("Đăng xuất tài khoản thành công").then(() => {
  ...
  const userMenu: MenuProps["items"] = useMemo(() => {
    return compact([
      ...
      isMobile && {
        ...
        label: (
          <Link href={AuthRouteUtils.toInternalWallet()}>
            Quản lý tài chính
          </Link>
        ),
  ...
  }, [isLoggingOut, isMobile, profileTitle]);
  ...
  // const sizeChat = storeNotify.numberChat;
