...
import JobTermsAndConditionsV2 from "@/src/screens/JobScreen/JobFormScreen/components/steps/confirm_and_register_tep/JobTermsAndConditionsV2";
import PartnerTermsAndConditions from "../../../components/PartnerTermsAndConditions";
import { useMemo } from "react";

interface Step12PersonalPageProps {
  form: FormInstance;
  selectBoxResource?: PartnerSelectBoxResource;
}

export const Step12PersonalPage: React.FC<Step12PersonalPageProps> = ({
  form,
  selectBoxResource,
}) => {
  ...
  // New check for personal verification data
  const hasPersonalVerificationData =
    step9PersonalData?.cardNumber ||
    step9PersonalData?.taxCode ||
    step9PersonalData?.frontCard ||
    step9PersonalData?.backCard ||
    step9PersonalData?.portraitCard;

  const hasPaymentData =
    step10Data?.bankId ||
    step10Data?.accountNumber ||
    step10Data?.qrCode ||
    step10Data?.bankAccountName; // Thêm tên tài khoản vào điều kiện

  const hasSkillsData = step3V2Data?.selectedSkillIds?.length > 0;
  const hasLanguageOrLocationData =
    step8Data?.languageIds?.length > 0 || step8Data?.locationIds?.length > 0;
  const hasReferralCode = step11Data?.referralCode;
    ...
    <div style={{ padding: "0 0" }}>
      ...
      <Row gutter={[32, 32]}>
        ...
        <Col span={24} md={12}>
            ...
            <>
              ...
              {step4Data?.length > 0 && (
                <div style={{ marginBottom: "16px" }}>
                  ...
                  {step4Data.map((exp: any, index: number) => (
                    <div
                      key={index}
          ...
          <Divider />
          {(hasPersonalVerificationData || hasPaymentData) && (
            <>
              <Typography.Title level={4} style={{ color: "#09993E" }}>
                Xác minh & Thanh toán (Bước 9 & 10)
              </Typography.Title>
              {hasPersonalVerificationData && (
                <div style={{ marginBottom: "16px" }}>
                  <Typography.Text strong>Xác minh cá nhân:</Typography.Text>
                  <div style={{ paddingLeft: "15px" }}>
                    {step9PersonalData?.cardNumber && (
                      <Typography.Text style={{ display: "block" }}>
