...
export const getServerSideProps: GetServerSideProps = async (
  ...
  try {
    ...
    return {
      props: {
        data: ValidatorUtils.removeEmptyFields(jobDetailResource),
...
export default function Component(
  ...
  const jobName = trim(job?.name) || "Công việc freelance";
  const partnerName =
    trim(job?.partner?.name) ||
    trim(job?.partnerName) ||
  ...
  const canonicalPath = (() => {
    const raw = trim((props as any)?.resolvedUrl, "");
  ...
  const JOB_DETAIL_JSONLD = {
    ...
    "@graph": [
      ...
      // JobPosting
      {
        ...
        employmentType: job?.employmentType || job?.typeName || "CONTRACTOR", // freelance
